...
void init_ddr(FILE *fp, long size){
    fseek(fp, 0, SEEK_SET);
    uint16_t bank = 0;
    uint16_t row = 0;
    uint32_t size_per_row = SDRAM_NR_COLS * 2;
    while(size > size_per_row){
        if (fread(&sdram[bank][row][0][0], size_per_row, 1, fp))
            size -= size_per_row;
        if (bank == SDRAM_NR_BANKS - 1)
            row = (row + 1) % SDRAM_NR_ROWS;
        bank = (bank + 1) % SDRAM_NR_BANKS;
    }
    fread(&sdram[bank][row][0][0], size, 1, fp);
}