...
class ID(config: RVConfig) extends Module with CsrConsts with ExceptionCodes {
  ...
  if (config.simulation) {
    RawClockedVoidFunctionCall("ebreak", Option(Seq("inst")))(
      clock,
      !io.toEXE.bits.nop,
      inst
    )
  ...
  io.toEXE.bits.decodeBundle.memRead := idu.io.decodeBundle.memRead & Fill(
    4,
    !io.toEXE.bits.nop
  )
  io.toEXE.bits.decodeBundle.memWrite := idu.io.decodeBundle.memWrite & Fill(
    4,
    !io.toEXE.bits.nop
  )
  ...
  val newestDataGetable = Mux(
    decodeBundle.aluSrc1 === OpASrc.RS1 || decodeBundle.aluSrc2 === OpBSrc.RS2,
  ...
  if (config.simulation) {
    RawClockedVoidFunctionCall(
      ...
      clock,
      enable = io.toEXE.fire && !hasFired && idu.io.legal && !io.toEXE.bits.nop,
      Cat(0.U(4.W), idu.io.decodeBundle.fuType)
    )
  ...
  if (config.trace_enable) {
    ...
    io.toEXE.bits.ftrace.rs1 := Mux(
      decodeBundle.aluSrc1 === OpASrc.PC,
      0.U,
      rs1Addr
    )
  }
