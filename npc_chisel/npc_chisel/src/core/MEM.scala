...
class MEM(config: RVConfig) extends Module with ExceptionCodes {
  ...
  val memRes = PriorityMux(
    Seq(
      byteWise(3) -> rdataShift,
      byteWise(1) -> Cat(
        Fill(16, rdataShift(15) & memSignedExt),
        rdataShift(15, 0)
      ),
      byteWise(0) -> Cat(
        Fill(24, rdataShift(7) & memSignedExt),
        rdataShift(7, 0)
      )
    )
  )

  val memWdata = PriorityMux(
    Seq(
  ...
  switch(state) {
    is(MEMState.IDLE) {
      when((ren || wen) && align) {
        ...
        when(io.toWB.fire || io.req.fire) {
          reservation := Mux(
            controlSignals.fuTypeAMO && controlSignals.amoOp === AMOOp.SC,
            0.U,
            reservation
          )
        }
    ...
    is(MEMState.AMO_LAUNCH_WRITE_REQ) {
      ...
      state := Mux(
        io.req.fire,
        MEMState.AMO_WAIT_WRITE_RESP,
        MEMState.AMO_LAUNCH_WRITE_REQ
      )
    }
