...
class Memory(config: RVConfig) extends Module {
    ...
    if (config.simulation){
        rdata := RawClockedNonVoidFunctionCall(
            ...
            io.clk, enable = io.dbus.ar.fire, io.dbus.ar.bits.addr
        )
        ...
        val wmask = Seq(io.dbus.w.bits.strb(0).asBool, io.dbus.w.bits.strb(1).asBool, io.dbus.w.bits.strb(2).asBool, io.dbus.w.bits.strb(3).asBool)
