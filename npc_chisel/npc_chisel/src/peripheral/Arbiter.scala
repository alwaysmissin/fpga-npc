...
class Arbiter(config: RVConfig, respCacheEnable: Boolean = true) extends Module {
    ...
    if (respCacheEnable){
        ...
        io.out.r.ready := Mux(io.out.r.bits.id === 0.U, rChannelIBus.ready, rChannelDBus.ready)
        ...
        io.dbus.wResp.valid     := bChannelQueue.valid
        io.dbus.wResp.bits.resp := bChannelQueue.bits.resp
        io.out.r.ready := Mux(io.out.r.bits.id === 0.U, io.ibus.resp.ready, io.dbus.rResp.ready)
        io.out.b.ready := bChannel.ready
    }
    ...
    io.out.ar.bits.addr := Mux(reqLock, 
                                ...
                                Mux(io.dbus.req.valid && !io.dbus.req.bits.wr, io.dbus.req.bits.addr, io.ibus.req.bits.addr))
    io.out.ar.bits.id   := Mux(reqLock, Mux(selDBus, 1.U, 0.U), Mux(io.dbus.req.valid && !io.dbus.req.bits.wr, 1.U, 0.U))
    io.out.ar.bits.burst := AXIBURST.INCR
    io.out.ar.bits.len   := Mux(reqLock, Mux(selDBus, 0.U, io.ibus.req.bits.len), Mux(io.dbus.req.valid && !io.dbus.req.bits.wr, 0.U, io.ibus.req.bits.len))
    io.out.ar.bits.cache := DontCare
    io.out.ar.bits.lock  := DontCare
    io.out.ar.bits.prot  := DontCare
    io.out.ar.bits.qos   := DontCare
    io.out.ar.bits.size  := Mux(reqLock, Mux(selDBus, io.dbus.req.bits.size, 2.U), Mux(io.dbus.req.valid && !io.dbus.req.bits.wr, io.dbus.req.bits.size, 2.U))
}
