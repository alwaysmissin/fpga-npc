...
package utils.nutshellUtils

import chisel3._
import chisel3.util._

object WordShift {
  def apply(data: UInt, wordIndex: UInt, step: Int) =
    (data << (wordIndex * step.U))
}
...
object GenMask {
  def apply(i: Int, j: Int): UInt = ZeroExt(Fill(i - j + 1, true.B) << j, 32)
  def apply(i: Int): UInt = GenMask(i, i)
}
