...
class ALU(config: RVConfig) extends Module{
    ...
    val cmp = 
        Mux(io.opA(config.xlen - 1) === io.opB(config.xlen - 1), sum(config.xlen - 1), Mux(io.op(1), io.opB(config.xlen - 1), io.opA(config.xlen - 1)))
    val shamt = io.opB(4, 0).asUInt
    val shin  = Mux(io.op(3), io.opA, Reverse(io.opA))
    val shiftr = (Cat(io.op(0) && shin(config.xlen - 1), shin).asSInt >> shamt)(config.xlen - 1, 0)
