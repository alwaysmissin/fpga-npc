...
class ICache(
    config: RVConfig,
    icache: CacheBasicConfig,
    skipSRAM: Boolean = true,
    burstEnable: Boolean = true
) extends Module {
  assert(
    icache.ways <= 2,
    "ICache only support 2-way-associate cache or direct map cache"
  )
  ...
  val dataRams = Seq.fill(icache.ways)(
    Module(
      new SDPRAM_SYNC(icache.blocks * icache.blockWords, UInt(config.xlen.W))
    )
  )
  ...
  val dataRams_waddr =
    Seq.fill(icache.ways)(WireDefault(0.U.asTypeOf(dataRams(0).io.waddr)))
  val dataRams_wdata =
    Seq.fill(icache.ways)(WireDefault(0.U.asTypeOf(dataRams(0).io.wdata)))
  ...
  val preIF = {
    ...
    indexPreIF := Mux(
      io.arFromIF.fire,
      addr(icache.indexRangeHi, icache.indexRangeLo),
      req.addr(icache.indexRangeHi, icache.indexRangeLo)
    )
    wordOffsetPreIF := Mux(
      io.arFromIF.fire,
      addr(icache.wordOffsetRangeHi, icache.wordOffsetRangeLo),
      req.addr(icache.wordOffsetRangeHi, icache.wordOffsetRangeLo)
    )
  ...
  val IF = {
    ...
    val hits = blockValids.zip(infoResp.tags).map { case (valid, t) =>
      valid && t === tag
    }
    when(state === IDLE) {
      hit := hits.reduce(_ || _)
  ...
  val burstable =
    if (burstEnable) req.addr >= 0x10000000L.U && req.addr < 0x1fffffffL.U
    else false.B
  when(io.fencei) {
  ...
  switch(state) {
    is(IDLE) {
      when(reqValid && !hit && !IFSkipCache) {
        ...
        arToMemW.bits.addr := Cat(
          req.addr(31, icache.offsetWidth),
          0.U(icache.offsetWidth.W)
        )
        ...
        when(io.arToMem.fire) {
          ...
          state := {
            if (icache.blockWords == 1) COMMIT else Mux(burstable, COMMIT, READ)
          }
        }
      }
    }
      ...
      when(io.rFromMem.fire) {
        for (i <- 0 until icache.ways) {
          when(i.U === replaceWay) {
            ...
            dataRams_wdata(i) := io.rFromMem.bits.rdata
              .asTypeOf(dataRams(0).io.wdata)
